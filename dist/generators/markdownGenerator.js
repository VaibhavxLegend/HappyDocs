...
function generateMarkdown(endpoints, config, workspaceRoot) {
    const groups = groupByTag(endpoints);
    const authEndpoints = endpoints.filter((endpoint) => endpoint.authentication);
    const gaps = endpoints.flatMap((endpoint) => endpoint.unresolvedItems.map((item) => ({ endpoint, item })));
    const lines = [
        `# ${config.apiTitle}`,
        "",
    ...
    for (const [tag, taggedEndpoints] of groups) {
        ...
        for (const endpoint of taggedEndpoints)
            appendEndpoint(lines, endpoint, workspaceRoot);
    }
    lines.push("## Documentation gaps", "");
    if (!gaps.length)
        lines.push("No unresolved route metadata was detected.");
    else {
...
function appendEndpoint(lines, endpoint, workspaceRoot) {
    ...
    if (endpoint.aiEnrichment?.accepted)
        lines.push("> AI-assisted text and examples are marked as suggestions accepted by the user.", "");
    lines.push(`- Confidence: **${endpoint.confidence}**`);
    lines.push(`- Source: \`${workspaceRoot ? (0, pathUtils_1.relativeTo)(workspaceRoot, endpoint.source.filePath) : endpoint.source.filePath}:${endpoint.source.line}:${endpoint.source.column}\``);
    ...
    if (endpoint.parameters.length) {
        ...
        for (const parameter of endpoint.parameters)
            lines.push(`| \`${parameter.name}\` | ${parameter.location} | ${parameter.type} | ${parameter.required ? "Yes" : "No"} | ${parameter.description ?? ""} |`);
        lines.push("");
    }
    if (endpoint.requestBody) {
        lines.push("#### Request body", "", `Content type: \`${endpoint.requestBody.contentType}\``, "", "```json", JSON.stringify(endpoint.requestBody.example ?? exampleForSchema(endpoint.requestBody.schema), null, 2), "```", "");
    }
    lines.push("#### Responses", "");
    for (const response of endpoint.responses) {
        lines.push(`- **${response.statusCode}** — ${response.description}`);
        if (response.schema || response.example !== undefined)
            lines.push("", "```json", JSON.stringify(response.example ?? exampleForSchema(response.schema), null, 2), "```");
    }
...
function exampleForSchema(schema) {
    ...
    if (schema.type === "object" || schema.properties)
        return Object.fromEntries(Object.entries(schema.properties ?? {}).map(([key, value]) => [key, exampleForSchema(value)]));
